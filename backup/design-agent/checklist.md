...
- [ ] Ask: **what does their Monday look like?** Write it out
- [ ] Identify the **2–3 jobs** the page must do
...
- [ ] `npm run dev` running, port noted
- [ ] Auth/password gate (if any) handled
- [ ] Browser automation launched and verified (take a first screenshot to confirm)
- [ ] Service worker unregistered + caches cleared
- [ ] Dev server output being monitored
- [ ] Task list set up with main work items
...
- [ ] **KEEP** — block is already the strongest it can be without external input *(use on <20% of blocks; any more means under-auditing)*
...
- [ ] Any sentences that would sound weird at a diner? Rewrite.
...
- [ ] Run the overflow detection script to find any element wider than viewport
- [ ] Hero text container has `min-w-0 w-full` on the grid item
- [ ] Any `aspect-video` element with `min-h-[Xpx]` changed to `sm:min-h-[Xpx]`
- [ ] Any grid explicitly uses `grid-cols-1` at base (not just `lg:grid-cols-2`)
...
**If the answer is "nothing" or "another section covers it," cut the section.**
...
- [ ] At least 2 blocks do NOT use the dominant rounded-2xl bordered-card pattern
...
- [ ] Section-headline scale varies (not every one the same `display-md`)
...
- [ ] Gradient-accent-word headlines per page: 1 (flag if ≥3)
...
**Do not overcorrect into:** wobbly borders, random rotations, novelty interactions. Quirky-for-the-sake-of-quirky is just a different AI tell. Goal is *intentional restraint*, not monastic minimalism and not theatrical quirkiness.
...
| Caching issues hiding your changes | Add `?bust=N` to every navigation; clear SW |
...
| Mixing icon libraries | Pick one source and stick to it |
| Three dark sections on one page | Max 2, alternate with light sections |
| Jargon slipping through | Run the forbidden-word grep before every merge |

---

## The 10-second page test

After you think you're done, do this:
...
- One or two specific moments (a recognition wall, a price reveal, a specific headline)
