...
Read `.claude/pending-learnings.txt` if it exists. This file is written by `Stop`/`PostToolUse` hooks capturing failure output from the previous session (lint errors, test failures, type errors).
...
Read `.claude/state.json` to find the current in-progress task.
...
- Check its "confidence map" — LOW-confidence steps should trigger a re-plan offer
...
Say so and suggest `/sync`, `/pull-task`, or `/batch` depending on intent.
