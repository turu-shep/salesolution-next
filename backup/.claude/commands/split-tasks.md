...
2. Read ROADMAP.md to understand current project state and what's already built
3. Read relevant module docs in `docs/modules/` for business rules and gotchas
4. Read relevant convention docs in `docs/conventions/` for patterns
5. Search the codebase thoroughly to understand:
   ...
   - What subscription/feature gating applies
   - What roles (customer, consultant, contractor, shop) are affected
...
**Scope:** booking | auth | subscription | video | org | profile | dashboard | sop | search | settings | navigation
...
- Database changes (table, columns, RLS policies, migrations)
- API endpoints (method, path, request/response shapes)
- Redux state changes (which slices, new selectors)
- Subscription tier gating (which tier, use `useFeatureAccess()`)
...
- [ ] E2E test: describe Playwright scenarios (if user-facing)
...
- Supabase dashboard configuration (RLS policies review, DB triggers)
- Stripe dashboard setup (products, prices, webhooks)
- Third-party service configuration (Daily.co, Resend, etc.)
- DNS/domain changes
...
- **`manual`**: >80% of the work requires human judgment, configuration, or actions outside the codebase (e.g., Stripe dashboard setup, design review, copy writing)
- **`hybrid`**: >50% manual work but has significant code components (e.g., implement UI but needs design decisions, write API but needs Supabase dashboard config)
