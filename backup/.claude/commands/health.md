...
**Tests**
- Run the project's test runner only if it completes in <30s
- Otherwise skip and note "test runner slow — skipped"
- Coverage if available from an existing report file (don't run the coverage tool)
...
|--------|----------------|-----------|------------|
| auth-pairing | 12 | 1 | 3d ago |
| migrations-rollback | 3 | 0 | 8d ago |
| no-hardcoded-secrets | 0 | — | never (30d) |
...
- **Posting the report is optional.** If your project has a dev-ops Slack channel or a Linear dashboard doc, configure posting in CLAUDE.md. Otherwise the file in `.claude/health/` is the artifact.
