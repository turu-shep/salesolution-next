...
```
/batch PROJ-73 PROJ-71 PROJ-74 PROJ-19 PROJ-75
/batch Implement PROJ-73, PROJ-71, and PROJ-74 in parallel worktrees
/batch all In Progress issues assigned to me
/batch PROJ-73,PROJ-71,PROJ-74 --review-plans
/batch PROJ-73 PROJ-71 --no-ship
/batch PROJ-73 PROJ-71 --pair
/batch PROJ-73 PROJ-71 --dry-run
```
...
- Confirm the parsed list with the user:
  ...
  Batch target (N issues):
    1. PROJ-73 — <title> (priority: High, spec: exists)
    2. PROJ-71 — <title> (priority: Medium, spec: missing)
    ...
  Flags: --review-plans
  Max parallel: 5
...
- For each issue, check `.claude/policies/*.md` — if any policy with `requires: ["human-approval"]` or `autopilot: block` matches an issue's likely target files, surface it and require explicit acknowledgment
- Write a batch manifest to `.claude/batches/<YYYY-MM-DD-HH-MM>.json`:
  ```json
  {
    "ts": "...",
    "base": "main",
    "issues": ["PROJ-73", "PROJ-71", ...],
    "flags": ["--review-plans"],
    "status": "planning",
    "subagents": {}
  }
...
For each issue, launch a sub-agent via the Agent tool with `isolation: "worktree"`. Each sub-agent gets a self-contained prompt — it does NOT share context with the main batch agent.
...
Follow the rules in .claude/rules/*.md and the conventions in CLAUDE.md.
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
PROJ-73   ✅ shipped  7/7    https://github.com/.../123   3 local decisions, 1 local learning
PROJ-71   ✅ shipped  4/4    https://github.com/.../124   -
PROJ-74   ⚠ blocked  2/5    -                            Needs design decision on X
PROJ-19   ✅ shipped  3/3    https://github.com/.../125   -
PROJ-75   ❌ failed   1/6    -                            Test suite broke, reverted
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Shipped: 3 | Blocked: 1 | Failed: 1 | Total: 5
...
```json
{"ts":"...","type":"batch","manifest":".claude/batches/<file>.json","issues":5,"shipped":3,"blocked":1,"failed":1,"duration_s":N}
```
...
- **Urgent production fixes** — do those interactively for fastest turnaround
