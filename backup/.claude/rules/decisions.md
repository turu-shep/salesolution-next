...
## How entries get into the log (v2.4: command-driven)
...
```json
{"ts":"2026-04-10T14:32:00Z","issue":"PROJ-123","step":3,"type":"approach-choice","decision":"Store idempotency key in request body","considered":["header","body","query param"],"chose":"body","why":"Client retries mangle headers when routed through CDN — confirmed by reproducing against staging","files":["app/api/payments/route.ts"]}
```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","issue":"PROJ-123","type":"approach-choice","decision":"...","chose":"...","why":"..."}' >> .claude/decisions.jsonl
```

Or use the Write/Edit tool.

**Commit the decision log.** It is shared team knowledge, unlike `.claude/state.json` which is per-developer local.
