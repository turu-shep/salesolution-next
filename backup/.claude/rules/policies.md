...
| Type | Role | Example |
|------|------|---------|
| **Learning** | "Here's a gotcha to be aware of" | "Stripe webhooks can arrive out of order — check idempotency keys" |
| **Module doc** | "Here's how this feature works" | `docs/modules/billing.md` describes the charge flow |
| **Policy** | "Here's a rule that MUST be followed, enforced by commands" | "Any change to `lib/auth/**` requires `/pair` mode" |
...
## Enforceable `requires:` vocabulary (v2.4: pruned)
...
| `no-hardcoded-secrets` | Grep the diff for common secret prefixes (`sk_`, `pk_live_`, `ghp_`, `xox[baprs]-`, `AKIA`, `AIza`, `-----BEGIN ... PRIVATE KEY-----`) and for string literals assigned to variables named `password`/`secret`/`token`/`api_key`. Skip test files and `.env.example`. |
| `rollback-sql-present` | For files matching the migration glob, check the file contains a rollback indicator: a line matching `/--\s*rollback/i`, a `down` or `rollback` function with non-trivial body, or a paired `down.sql`/`<name>_down.sql` file in the same directory. |
...
**That's the whole list.** The earlier draft of this vocabulary included `test-coverage-min-N`, `changelog-entry`, and several others. Those are aspirational — commands don't have reliable ways to verify them without project-specific tooling. If you want test coverage enforcement, use your CI; don't put it in a policy file.
...
```markdown
## How commands verify it
Run: `grep -r 'TODO' app/api/auth/ | wc -l`
Fail if the result is > 0. (No TODOs allowed in auth endpoints.)
```
...
1. The override MUST be explicit. Require the user to type the policy name: `override policy auth-pairing` plus a reason.
...
**Good policies (enforceable with the built-in vocabulary):**
- Auth changes require `/pair` and a spec → `requires: ["pair-mode", "spec-exists"]`
- Migration files need rollback SQL and human approval → `requires: ["rollback-sql-present", "human-approval"]`
