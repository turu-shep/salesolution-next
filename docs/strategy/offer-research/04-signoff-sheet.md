...
| **MED-hero** | Medical pillar hero (`/industries/medical-aesthetics/`) | **SIGNED 2026-07-07: D+F combined** ("A working system, not another campaign." + the no-assumption/audit-decides lede; "quoted plan" not "plan"; no guarantee mention in the hero). Final wording in medical spec §5.1 PICKED block. A/B/C rejected (pain-guess heroes); E/F kept as alternates. | Shipped — the medical pillar migration is complete | **SIGNED** |
...
| **D-C4** | Liori Diamonds consent ask | **CONSENT RECORDED (founder, 2026-07-07: "you get my consent").** Named-client line shipped on the consumer pillar close, scoped strictly to what we run for them (search + content) — no outcome claims, no numbers. A full named CASE STUDY still needs attested real numbers (claims row) before it exists anywhere. | Consumer pillar named proof — shipped. TwoRevenueLines PROOF-SLOT stays empty until real dashboard data. | **SIGNED / partially consumed** |
| **FGO-24h** *(flag from the batch-3 services pass)* | FGO "written quote in 24 hours" vs the 48h SOW standard | **RESOLVED (founder, 2026-07-07: "I can get 24h").** FGO's standalone quote turnaround STAYS 24 hours on all FGO surfaces (`FullGrowthTierCard`, FGO page, quote funnel); 48h remains the install/cylinder-SOW standard. Deliberate tiering: the premium tier gets faster paper. Template comment updated. | Closes the batch-3 harmonization flag; no code change needed | **SIGNED** |
