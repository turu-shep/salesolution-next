...
**✅ Phase 2 done — `/services/` hub = the cylinder library** (system-first hero "The parts that stop your leaks. One engine."; the "one machine, not a menu" argument — 60/40 + "Five agencies, no accountability." — above the grid; the full 12-cylinder catalog incl. "Coming soon" via the shared SixCylinders + CYLINDER_GROUPS, so the hub and the product page show the identical catalog; PickAService → "Where does your system start?"; pricing demoted). Validated via the visual loop. Also **de-muted the shared two-tone headlines** (ServicesHero, SixCylinders, EngagementShapes → full-contrast — also fixes the product + service-page heroes) and **unified the primary CTA to "Book a Growth Call."**

**↳ Refined 2026-06-29 — the engine-as-base story.** Reworked the hub so it *decommoditises* the services (we're a system, not six commodity vendors) and *de-vagues* the engine (the system is abstract + ~3 months of work, so the recognizable disciplines make it concrete). New compact `ServicesHubHero` keeps the value visible fast (a full-screen headline was burying it): a short hook ("One system, not six vendors.") with the recognizable disciplines as scannable tags right under it. New `components/sections/services/EngineBase.tsx` (dark) reveals the **Revenue Engine as the installed base** (~90 days) with the **cylinders as the parts that bolt on top** — a layered visual: pillar-grouped part-chips → a connector rail → the weighted engine base block (Full Growth Ownership excluded from the chips; it's the whole-engine tier, not a bolt-on part). Thesis line: "A part on its own is just a tool. Bolted to the engine, it compounds." Arc (value-first): compact hero → **EngineBase** (engine + parts, brought up so the value lands on the first screen) → SixCylinders (the parts catalog) → HowServicesCombine (supporting: why + combinations + spend, demoted from the prime second-screen slot) → PickAService → EngagementShapes → FAQ. `/services/` = the concrete work + the system that makes it not-a-commodity; `/revenue-engine/` = how the whole system works. Validated via the visual loop (two passes: the engine visual, then the value-first hero compaction + reorder per founder feedback).
...
**Status (2026-06-30): ✅ reframe + niche cross-links.** The index was already a working four-pillar router (`IndustriesShowcase`: pillar-level image cards, motion-color-graded). Reframed the **hero** to the canonical "Pick the engine for your business." + a one-engine-per-business lede that names the two motions; aligned the **metadata** to pillar level. Then **enriched each card with its niches as small cross-link chips** — linked where a niche page exists (**Dental → `/revenue-engine/dentists/`** today), plain labels otherwise (Roofing/HVAC/Med spa/Jewelry/etc. light up as Phase 6 builds them). The card was restructured from one wrapping `<Link>` into a container with sibling links (image + CTA → hub, niche chips → niche pages) so the niche anchors are valid. tsc/lint clean, route 200; chips validated on the industrial (label) + medical (Dental link) cards.
- **Case-studies band at the bottom: HELD (not appropriately filled).** Sanity has **10 published case studies, all industrial; zero for home/medical/consumer, and 5/10 are the "Northern Hydraulics" naming landmine** (`case-studies/fact-ledger.md`). A band would be lopsided + surface the landmine. Revisit when there's ≥1 non-industrial study and the Northern Hydraulics composite is renamed. (The Sanity `getAllIndustries()` taxonomy — verticals + sub-niches + live `caseStudyCount` — is the right data source when it's time.)
- **Pillar destinations today:** industrial → `/industries/industrial-distribution/` (live); home/medical/consumer cards still point at `/revenue-engine/{home-services,medical,local-retail}/` until the **Phase-5 remap**.
- **Left to Phase 5 (deliberately not touched here):** the `local-retail` card's orange/book-jobs coloring — the **consumer motion-flip scrub** rides with the URL remap. `IndustriesShowcase` still keeps its own AUDIENCES copy mirroring homepage `WhoWeServe`.
...
**Status (2026-06-30): ✅ COMPLETE.** All 3 new thin pillars built + the atomic URL remap shipped (commit `717b2b9`).
- **Pillars:** `/industries/home-services/` (book-jobs, Revenue Leak Audit, day-90, hosts `#audit`), `/industries/medical-aesthetics/` (book-appts, day-90 + HIPAA woven through 4 surfaces), `/industries/consumer-brands/` (**motion-flipped to sell-product** — Book a Growth Call, no guarantee, EngagementShapes price bands, "we" voice). Industrial pillar already existed (the fat template). Each new pillar emits breadcrumb JSON-LD + canonical at `/industries/`.
- **Remap (atomic):** 308 redirects in `lib/redirects.ts`; deleted the 3 old `/revenue-engine/{home-services,medical,local-retail}/` dirs (kept the product page + dentists); repointed every internal link (nav full-rework + Dentists child, RevenueFooter, HeroProbe, WhoWeServe, IndustriesShowcase [+ consumer color flip], FinalCTARail, product-page refs, strategy/niches, llms.txt, VerticalFork); `registry.ts` + reconcile-test pin updated; dentists got a breadcrumb (no 301, highest-equity URL). Verified: tsc clean, reconcile 2/2, zero straggler links, pillars+dentists 200, old paths 308 → correct destinations.
- **Post-build polish (2026-06-30): ✅ done.** (1) **AIOverviewMockup landmine** — Forge Fluid Power + Cedar & Co Roofing renamed to clearly-generic examples with an "Illustrative example" badge (`4db2af6`); the other cited businesses (Plantation Dental, Modern Wood Flooring, Liori Diamonds) are real clients and stay. (2) **cylinder de-count** — the catalog is 12, so all "six cylinders / six services / all six" copy on `/revenue-engine/` + the industrial pillar dropped the count per house style; the dim BRING/CONVERT/RETAIN + report-card eyebrows lifted `text-ink-400 → ink-300` for AA on dark (`918abed`). (3) consumer card image re-graded warm → brand-blue (`ddcd5e0`).
- **Still open:** (4) a styled "Featured engines" nav divider + the Jewelry niche await Phase 6. (5) **`/services/` stale counts** (owned by the parallel `/services/` chat — see [docs/handoff/06/29/services-hub.md](../../handoff/06/29/services-hub.md)): `ServicesTabs` "Six services. One operator-led team." (renders 5 tabs), the `/services/` hub + `consumer-brands` metadata "six vendors" vs the site-wide "five vendors" frame, `EngagementShapes`/`HowServicesCombine` "six vendors", and `FullGrowth*` "five services" understating the 12-catalog. (6) dead components `ServicesByLeak`/`ServicesSystem` carry stale "six" comments — low priority.
