...
**↳ Refined 2026-06-29 — the engine-as-base story.** Reworked the hub so it *decommoditises* the services (we're a system, not six commodity vendors) and *de-vagues* the engine (the system is abstract + ~3 months of work, so the recognizable disciplines make it concrete). The hero now leads with the disciplines as the concrete hook ("SEO, ads, content, your site. One system, not six vendors."). New `components/sections/services/EngineBase.tsx` (dark) reveals the **Revenue Engine as the installed base** (~90 days) with the **cylinders as the parts that bolt on top** — a layered visual: pillar-grouped part-chips → a connector rail → the weighted engine base block (Full Growth Ownership excluded from the chips; it's the whole-engine tier, not a bolt-on part). Thesis line: "A part on its own is just a tool. Bolted to the engine, it compounds." Arc: hero → HowServicesCombine (why one system) → **EngineBase** (the base + parts) → SixCylinders (the parts catalog) → PickAService → EngagementShapes → FAQ. `/services/` = the concrete work + the system that makes it not-a-commodity; `/revenue-engine/` = how the whole system works. Validated via the visual loop. **Open:** hero runs ~4 lines (disciplines kept prominent over a shorter hero — founder's call if it should tighten further).
