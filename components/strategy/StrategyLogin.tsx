...
/**
 * Password gate for the private /strategy area. The layout renders this in place
 * of the page when a production request has no valid session cookie. Posts to
 * /api/strategy/login; on success the cookie is set and we reload into the area.
 * Mirror of components/sales/SalesLogin.tsx.
 */
export function StrategyLogin() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError('')
    try {
      const res = await fetch('/api/strategy/login', {
        method: 'POST',
  ...
  return (
    <main className="flex flex-1 items-center justify-center px-4 py-24">
      <form onSubmit={onSubmit} className="w-full max-w-sm">
        ...
        <h1 className="mt-2 text-lg font-medium text-ink-900">Strategy — private</h1>
