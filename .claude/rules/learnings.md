...
This project uses `.claude/learnings/` to capture hard-won knowledge from failures, bugs, and surprises. Learnings are the *negative* knowledge base — things that went wrong and what to do about them. The *positive* knowledge base lives in `docs/modules/` (see `.claude/rules/module-context.md`), with the standing Sanity/content gotchas already documented in `prompts/_CONTEXT.md`.
...
- Things already documented in CLAUDE.md / AGENTS.md, `prompts/_CONTEXT.md`, module docs, or existing rules
...
Severity is **not** auto-assigned. If a learning needs a severity marker, a human should add it later, either manually or via `/retro` when the human reviews patterns. (Exception: `/postmortem` writes `severity: high` learnings — those are anchored to a real incident, not a feeling.)
