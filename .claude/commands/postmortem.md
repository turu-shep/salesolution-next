...
"Incident" here covers anything that hurt production: site down or broken deploy, SEO regression (deindexed pages, broken JSON-LD/sitemap), a lead-funnel outage (form/API failures), a bad outbound-email send, or wrong published content/facts.

## Invocation patterns

```
/postmortem                                      # ask for incident details interactively
/postmortem "Lead form 500s started 2026-08-07 14:32 UTC on /api/lead"
...
- When was it noticed?
- What user-visible symptom? (500s, wrong data, slowness, missing content, wrong send)
- How was it detected? (user report, Sentry, GSC, analytics, accident)
- Is it resolved? If yes, how (rollback / hotfix / auto-recovered)?
- Estimated blast radius (visitors/leads/recipients affected, duration)

**From Sentry (if the DSN is configured):**
- Check the Sentry dashboard for the issue, stack trace, first seen, frequency
...
- Vercel deploys correlate with pushes to `main`
...
Rank by the strength of *concrete evidence*, not by a made-up probability number. Describe what you can see (file appears in stack trace, timestamp correlation, etc.) and let the human judge.
...
- Why did the assumption break in production but not locally / in preview? (common here: env vars set locally but not in Vercel; drafts visible locally via `perspective: 'raw'` but absent in production; ISR/cache serving stale pages)
...
## Blast radius
- Visitors/leads/recipients affected: N
- Duration: <minutes>
- Data integrity impact: <none / recoverable / lost>
- SEO impact: <none / rankings-at-risk / deindexed pages>
...
- The root cause was preventable by a mechanical check (missing env var, missing test, unregistered schema)
...
Print the summary with the postmortem path and open action items.

## Important

- **Blameless.** Postmortems focus on systems, not individuals. Frame root causes as "the review process didn't catch X", never as personal failure.
