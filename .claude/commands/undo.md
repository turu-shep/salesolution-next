...
- The step seeded Sanity documents → reverting code does NOT delete Sanity drafts; list the affected document IDs so the user can clean them in `/studio` (or via a script) explicitly
...
```json
{"ts":"...","issue":"SAL-123","type":"undo","step":N,"reverted_commits":["sha1","sha2"],"reason":"<user-provided or 'approach didn't work'>","supersedes":"<ts of the decision the undone step created, if any>"}
```
...
- Any Sanity drafts needing manual cleanup
...
If invoked for multiple issues at once (`/undo SAL-123 SAL-124`), process each in isolation — they may live on different branches or in different worktrees. Do NOT assume they share state. Report a table of results at the end.
