...
**Honest framing on "independence":** The reviewer subagent runs the same underlying model and shares training biases with the implementer — it's not as independent as a different human. What it DOES get you:
...
- High-stakes changes (lead funnel, public SEO surfaces, pricing/offer pages, gated areas, outbound-email sending scripts)
...
- When you want higher confidence without a full manual review
...
**b. Spawn the reviewer** as a subagent (use the `reviewer` agent type if available, otherwise `general-purpose`; no `model:` override — it inherits the session model). Pass it:
...
- Run the normal verification: lint, `npx tsc --noEmit` (lead-form baseline excepted), tests
...
```json
{"ts":"...","issue":"SAL-123","step":N,"type":"pair-session","chunks":N,"reviewer_issues":{"fail":N,"warn":N,"pass":N},"learnings_created":N}
```
...
2. Convention compliance — does it follow CLAUDE.md / AGENTS.md and relevant rules?
3. Security — any injection, missing validation, exposed secrets, unsafe defaults?
4. Edge cases — null handling, empty states, error paths, draft-vs-published content?
...
- **Keep chunks small.** A chunk larger than ~150 lines dilutes the reviewer's attention. Split it.
