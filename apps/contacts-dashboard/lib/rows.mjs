 ...
 * 2. `pool` is dropped outright. (It used to feed a derived US/Non-US country;
 *    that left with the G2 non-us drop, 2026-08-10 — the base is US-only.)
 *    `id`, `pool`, `list_generation` and anything else off the whitelist never
 *    reach HTML, JSON, serialized props, or the export.
 */
...
/**
 * A database row -> the row the client is allowed to see: `key` (opaque) and
 * the whitelist columns — each an explicit null when absent, so the shape
 * never varies.
 */
export function toClientRow(row) {
  const r = row ?? {}
  const out = { key: opaqueKey(r.id) }
